...
卷弧线拆解任务 `expand_volume_arcs` 的模型输出上限从 `16384` 调整为 `1280000`，用于降低复杂卷、多弧线、长 JSON 输出时被截断的概率。

同时保留拆弧线提示词中的输出长度约束，避免模型把每条弧线写成完整章节细纲或人物小传，导致即使提高 token 上限仍然输出过长。

注意：不同模型供应商可能会按自身模型能力、接口限制或账户配置对 `max_tokens` 做上限裁剪。如果模型调用记录里仍出现“出 token 接近供应商实际上限”，说明是供应商侧限制，需要继续改成分批生成弧线。
