...
卷弧线拆解任务 `expand_volume_arcs` 的模型输出上限从 `16384` 调整为 `393216`，这是当前接入模型接口允许的最大 `max_tokens` 值，用于降低复杂卷、多弧线、长 JSON 输出时被截断的概率。
...
注意：不同模型供应商可能会按自身模型能力、接口限制或账户配置对 `max_tokens` 做上限裁剪。当前接口报错确认有效范围为 `[1, 393216]`，因此不能继续设置为 `1280000`。如果模型调用记录里仍出现“出 token 接近供应商实际上限”，说明需要继续改成分批生成弧线。
