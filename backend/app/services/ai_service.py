...
【arc_step_refs 强制要求】
- 每个章节必须填写 arc_step_refs 字段，标明本章承载弧线的哪个台阶（从上面【弧线输入】的 arc_steps 中选择）。
- arc_step_refs 是数组，可以包含1个或多个台阶名称（如果本章跨越多个台阶）。
- 如果本章是桥接章、余波章或过渡章，不直接承载某个台阶，arc_step_refs 填 ["桥接过渡"] 或 ["余波处理"]。
- arc_step_refs 不能为空，必须明确本章在弧线中的位置和责任。
- 例如：如果 arc_steps 包含 "下村受冷" "村情摸底" "茶山试种"，第1章承载 "下村受冷"，则 arc_step_refs: ["下村受冷"]。
